...
def plot(x,y,**plt_kwargs):
    ...
    if 'style' in plt_kwargs:
        ax.plot(x, y,marker=plt_kwargs['style'])
    elif 'style' not in plt_kwargs:
        ax.plot(x,y)
    if 'axes_ticks' in plt_kwargs:
        if plt_kwargs['axes_ticks']=='default':
        ...
        if plt_kwargs['axes_ticks']=='x':
            xticks = x
            ax.set_xticks(xticks)
            ax.set_xticklabels(ax.get_xticks(), rotation=60, fontname='Times New Roman')

        if plt_kwargs['axes_ticks'] == 'y':
            ytick = y
            ax.set_yticks(ytick)
            ax.set_yticklabels(ax.get_yticks(), fontname='Times New Roman')
...
def scatter(x,y,**plt_kwargs):
    ...
    if 'style' in plt_kwargs:
        ax.scatter(x, y,marker=plt_kwargs['style'])
    elif 'style' not in plt_kwargs:
        ax.scatter(x,y)
    # if 'axes_ticks' in plt_kwargs:
    #     if plt_kwargs['axes_ticks']=='default':
